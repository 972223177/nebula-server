...
- [ ] CQ-01: 跨模块死代码清理（SendMessageStep 链 7 文件 + SequenceOverflowException + Dispatcher.scope）
- [ ] CQ-02: 生产安全加固（Redis 认证 + SSL sslMode + DatabaseConfig 密码泄露 + 密码环境变量注入）
- [ ] CQ-03: 服务层事务保护（ConversationService/FriendService 跨 Repository 写入事务）
- [ ] CQ-04: 数据一致性与竞态修复（好友双向竞赛 + SeqService 序列号重置 + Conversation memberCount 原子更新）
- [ ] CQ-05: 可靠性基础设施（JVM Shutdown Hook + 全局异常处理 + ConnectionId 刷新）
- [ ] CQ-06: 数据丢失修复（repository 唯一索引冲突入死信 + DeadLetterService compensate payload 传递 + 消息 payload 异步路径序列化）
- [ ] CQ-07: N+1 查询消除（ConversationService inviteMember/createGroup + repository 子查询改 JOIN）
- [ ] CQ-08: 业务逻辑修复（DeadLetterService 分页总数 + markPermanentFailed 死查询 + MessageService 未读计数）
- [ ] CQ-09: 启动健壮性（Server 启动 SSL 预校验 + init 链回滚 + ConfigLoader 配置校验 + 文件存在性检查）
- [ ] CQ-10: 日志与可观测性（println → Logger + 登录失败审计日志 + 配置文件缺失提示）
- [ ] CQ-11: 协程与线程安全（PushService blocking JPA + ChatServer @Volatile server + 非结构化 launch + RateLimiter/Mutex 泄漏）
- [ ] CQ-12: 状态管理一致性（conversations.type 值定义矛盾 + 好友申请状态魔法数字 + 会话类型枚举）
- [ ] CQ-13: 消息可靠性增强（clientMessageId 去重下沉 + Redis Stream/MySQL 原子性 + unique constraint 处理）
- [ ] CQ-14: 测试健壮性（Clock 接口抽取 + exceptions 路径测试 + mock 改进 + SnowflakeIdGenerator 反射替代）
- [ ] CQ-15: 代码质量清理（!! 空断言替换 + 死代码移除 + DRY 违反修复 + 异常类型精简）
