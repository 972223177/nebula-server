...
/**
 * 死信业务契约（2026-07-29 字面 Facade 改造，仿 `conversation/ConversationService`）。
 *
 * 所有公开方法声明在此接口；[DeadLetterServiceImpl] 实现之，聚合 Facade [DeadLetterService] 经 `by` 委托暴露。
 * 另实现 [DeadLetterCallback]（D-28 跨层桥接），故聚合 Facade 同时 `by` 委托两接口，桥接零改动。
 */
interface DeadLetterOperations {
    /** 创建毒消息死信记录（D-03 修复） */
    suspend fun createUnparseable(rawBody: Map<String, String>, reason: String): DeadLetterDTO
...
/**
 * 死信服务实现 — 提供死信记录的创建、补偿重试、查询和管理（Phase 10, D-76）。
 *
 * 职责：
 ...
 *
 * @param deadLetterDao 死信 DAO
 * @param txRunner JPA 事务运行器
 * @param messageQueueRepository 消息队列仓储（重新入队）
...
class DeadLetterServiceImpl(
...
) : DeadLetterOperations, DeadLetterCallback {
     ...
     * 毒消息死信回调实现（D-03 修复）。
     *
     * 由 repository 模块在 Redis Stream 条目无法反序列化为 [DeadLetterEntity]
     ...
     * @return 创建的死信 DTO
     */
    override suspend fun createUnparseable(
        rawBody: Map<String, String>,
        reason: String
    ): DeadLetterDTO {
     ...
     * @return 本次补偿处理的死信数量
     */
    override suspend fun compensate(): Int {
    ...
    override suspend fun query(page: Int, pageSize: Int, status: String?): ListPage<DeadLetterDTO> {
        ...
        return txRunner.execute { em ->
            if (status.isNullOrBlank()) {
                ...
                ListPage(allItems.map { it.toDeadLetterDTO() }, total)
            } else {
     ...
     */
    override suspend fun markPermanentFailed() {
