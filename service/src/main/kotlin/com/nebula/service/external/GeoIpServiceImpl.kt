...
import com.nebula.service.external.GeoIpService.GeoIpResult
...
/**
 * IP 地理定位业务契约（2026-07-29 字面 Facade 改造，仿 `conversation/ConversationService`）。
 *
 * 所有公开方法声明在此接口；[GeoIpServiceImpl] 实现之，聚合 Facade [GeoIpService] 经 `by` 委托暴露。
 */
interface GeoIpOperations {
    /** 按客户端 IP 查询地理定位 */
    suspend fun locate(ip: String): GeoIpResult
}

/**
 * IP 地理定位服务实现（D-XX）—— 对接高德(Amap) IP 定位（国内可达，需 Key）。
 *
 * 职责：
...
class GeoIpServiceImpl(
     ...
     */
    override suspend fun locate(ip: String): GeoIpResult {
        return queryAmap(ip)
