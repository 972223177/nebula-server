...
/**
 * 消息业务契约（2026-07-29 字面 Facade 改造，仿 `conversation/ConversationService`）。
 *
 * 所有公开方法声明在此接口；[MessageServiceImpl] 实现之，聚合 Facade [MessageService] 经 `by` 委托暴露。
 * Handler 仅依赖 [MessageService]（即本接口），不感知实现拆分。
 */
...
/**
 * 消息业务服务实现（D-04 ~ D-06, D-09, D-11, D-13, D-15, D-74, D-78）。
 *
 ...
 *
 * 落库细节（幂等去重 / Redis Stream 落库）下沉到 [MessagePersistHelper]，Entity→Protobuf 映射下沉到
 * [com.nebula.service.chat.toChatMessage]（`MessageMappers.kt`），本类仅保留业务编排。
 */
class MessageServiceImpl(
    ...
    /** 落库辅助：幂等去重 / Redis Stream 落库（P2 抽取，D-04/D-06/D-09） */
     ...
     */
    override suspend fun sendMessage(req: SendMessageReq, senderUid: Long): SendMessageResult {
     ...
     * @return 消息列表响应
     */
    override suspend fun pullMessages(req: PullMessagesReq, userId: Long): PullMessagesResp {
        val conversationId = req.conversationId
     ...
     * @param userId 当前用户 ID
     */
    override suspend fun readReport(req: ReadReportReq, userId: Long) {
        val conversationId = req.conversationId
     ...
     * @return 消息总数
     */
    override suspend fun countByConversationId(conversationId: String): Long {
     ...
     */
    override suspend fun checkAndSetDedup(clientMessageId: String, senderUid: Long): Boolean {
...
data class SendMessageResult(
    ...
    /**
     * 序列化后的会话 Brief（懒加载响应字段），由 MessageServiceImpl 内部构造。
