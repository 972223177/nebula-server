...
/**
 * 会话消息序列号服务契约（2026-07-29 字面 Facade 改造，仿 `conversation/ConversationService`）。
 *
 * 所有公开方法声明在此接口；[SeqServiceImpl] 实现之，聚合 Facade [SeqService] 经 `by` 委托暴露。
 */
interface SeqOperations {
    /** 获取会话下一条序列号并自增 */
...
/**
 * 会话消息序列号服务实现 — 基于 Redis INCR 的序列号生成（Phase 10）。
 *
...
class SeqServiceImpl(
     ...
     * @return 新的序列号
     */
    override suspend fun nextSeq(convId: String, uid: Long): Long {
     ...
     */
    override suspend fun currentSeq(convId: String, uid: Long): Long {
        val redisKey = key(convId, uid)
     ...
     */
    override suspend fun tryRestoreSeq(convId: String, uid: Long, nextSeq: Long): Boolean {
        val redisKey = key(convId, uid)
     ...
     *
     * 原 [com.nebula.server.bootstrap.ServerBootstrap.recoverSequences] 直接依赖 repository 层的
     ...
     */
    override suspend fun recoverSequences(
        conversationSupplier: suspend () -> List<Pair<String, Int>>,
        msgCountByConv: suspend (String) -> Long,
        memberSupplier: suspend (String) -> List<Long>
    ): Int {
