...
/**
 * 敏感词库下载 Handler — method = "system/sensitive-word/download"（D-118）。
 *
 * 客户端启动后调用，分页拉取服务端内存中的敏感词列表，用于在本地构建过滤结构。
 * 无需登录（AuthInterceptor 白名单前缀 "system/sensitive-word" 跳过认证）。
...
class SensitiveWordDownloadHandler(
    ...
    /** method 路由：system/sensitive-word/download */
    override val method: String = "system/sensitive-word/download"
