...
class PullMessagesHandler(
     ...
     * 2. 成员身份检查：验证请求者是会话成员，非成员抛 NOT_MEMBER（D-07）
     * 3. 会话存在性检查：通过 conversationRepository.existsById() 验证会话是否存在（REVIEW-MEDIUM-9）
     * 4. cursor 特殊处理：cursor=0 时使用 Long.MAX_VALUE 作为有效游标（D-18, Pitfall 2）
     * 5. limit 范围约束：使用 .coerceIn(1, 100) 防止零大小查询和过大结果集（D-19, T-06-08）
     * 6. 执行游标分页查询：messageRepository.findMessagesBackward()
     * 7. Entity → ChatMessage 映射：包含 messageType 的 UNRECOGNIZED 保底处理（REVIEW-MEDIUM-11）
     *
    ...
    override suspend fun handle(req: PullMessagesReq): PullMessagesResp {
        ...
        // D-07: 成员身份检查 — 非成员拉取消息抛 NOT_MEMBER
